...
namespace QuanLyCaPhe.DataAccess
{
    class NhanVienDAL
    {
        ...
        {
            string caulenh = "select * from Nhanvien where tennv like N'" + tennv + "'";
            return dah.get_DaTaTable(caulenh);
        }
        public DataTable ListBySeachCCCD(int socmnd)
        {
            string caulenh = "select * from Nhanvien where socmnd = " + socmnd + "'";
            return dah.get_DaTaTable(caulenh);
        }
